...
_PLATFORMS = [
    ...
    {
        "key": "gewe",
        "label": "GeWe / WeChat API",
        "emoji": "💬",
        "token_var": "GEWE_APP_ID",
        "setup_instructions": [
            "1. Create or obtain a GeWe appId and API token from your GeWe service",
            "2. Outbound messages and media downloads call GeWe directly",
            "3. Inbound mode can be direct-callback, relay-callback, or relay-sse",
            "4. direct-callback does not require webhook-router; relay modes use webhook-router as a relay",
            "5. For relay-sse, configure the webhook-router app id/token and channel filter if needed",
        ],
        "vars": [
            {"name": "GEWE_TOKEN", "prompt": "GeWe API token (X-GEWE-TOKEN)", "password": True,
             "help": "Token used for GeWe API calls."},
            {"name": "GEWE_APP_ID", "prompt": "GeWe App ID", "password": False,
             "help": "The appId used by GeWe send/download APIs and callbacks."},
            {"name": "GEWE_BOT_WXID", "prompt": "GeWe bot WeChat wxid", "password": False,
             "help": "Required for reliable group @ routing; must match GeWe callback at-user wxids."},
            {"name": "GEWE_API_BASE_URL", "prompt": "GeWe API base URL (default: https://api.geweapi.com)", "password": False,
             "help": "Base URL for direct GeWe API calls."},
            {"name": "GEWE_INBOUND_MODE", "prompt": "Inbound mode: direct-callback, relay-callback, or relay-sse", "password": False,
             "help": "direct-callback receives GeWe callbacks directly; relay modes consume webhook-router callbacks/SSE."},
            {"name": "GEWE_CALLBACK_PORT", "prompt": "Callback server port (default: 8656)", "password": False,
             "help": "Port used by direct-callback and relay-callback modes."},
            {"name": "GEWE_CALLBACK_PATH", "prompt": "Callback path (default: /gewe/callback)", "password": False,
             "help": "HTTP path used by direct-callback and relay-callback modes."},
            {"name": "GEWE_CALLBACK_SECRET", "prompt": "Callback shared secret (optional)", "password": True,
             "help": "If set, callback requests must include X-Hermes-Gewe-Secret or X-Gewe-Secret."},
            {"name": "GEWE_RELAY_BASE_URL", "prompt": "Webhook-router base URL (default: https://hook.yunzxu.com)", "password": False,
             "help": "Used by relay callback/SSE modes."},
            {"name": "GEWE_RELAY_APP_ID", "prompt": "Webhook-router app ID for SSE (optional)", "password": False,
             "help": "Used to build /apps/<app_id>/events when GEWE_RELAY_SSE_URL is not set."},
            {"name": "GEWE_RELAY_APP_TOKEN", "prompt": "Webhook-router app token for SSE (optional)", "password": True,
             "help": "Bearer token for webhook-router SSE."},
            {"name": "GEWE_RELAY_CHANNEL", "prompt": "Webhook-router channel filter (optional)", "password": False,
             "help": "Only consume relay events from this channel."},
            {"name": "GEWE_ALLOWED_USERS", "prompt": "Allowed wxids (comma-separated, or empty)", "password": False,
             "is_allowlist": True,
             "help": "Restrict which WeChat users can interact with Hermes."},
            {"name": "GEWE_GROUP_REQUIRE_MENTION", "prompt": "Require @ mention in groups (true/false, default false)", "password": False,
             "help": "When true, group messages only process if the callback at-user list contains GEWE_BOT_WXID."},
            {"name": "GEWE_HOME_CHANNEL", "prompt": "Home channel wxid for cron/notifications (optional)", "password": False,
             "help": "wxid or chatroom id to deliver cron results and notifications to."},
        ],
    },
    {
        "key": "bluebubbles",
