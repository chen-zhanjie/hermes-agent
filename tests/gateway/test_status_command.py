...
@pytest.mark.asyncio
async def test_first_run_home_channel_onboarding_is_localized_in_chinese(monkeypatch):
    import gateway.run as gateway_run
    from agent import i18n
    ...
    session_entry = SessionEntry(
        session_key=build_session_key(_make_source(Platform.GEWE)),
        session_id="sess-1",
        created_at=datetime.now(),
        updated_at=datetime.now(),
    ...
    runner = _make_runner(session_entry, platform=Platform.GEWE)
    runner.session_store.load_transcript.return_value = []
    runner.session_store.has_any_sessions.return_value = False
    runner._run_agent = AsyncMock(
        return_value={
    ...
    monkeypatch.delenv("GEWE_HOME_CHANNEL", raising=False)
    monkeypatch.setattr(gateway_run, "_resolve_runtime_agent_kwargs", lambda: {"api_key": "***"})
    monkeypatch.setattr(
        "agent.model_metadata.get_model_context_length",
        lambda *_args, **_kwargs: 100000,
    ...
    result = await runner._handle_message(_make_event("hello", platform=Platform.GEWE))

    assert result == "ok"
    runner.adapters[Platform.GEWE].send.assert_awaited_once()
    onboarding = runner.adapters[Platform.GEWE].send.await_args.args[1]
    assert "Gewe 还没有设置主页频道" in onboarding
    assert "输入 /sethome" in onboarding
